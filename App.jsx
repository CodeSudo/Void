...
function App() {
  ...
  // --- PLAYBACK ENGINE WITH YOUTUBE FALLBACK ---
    ...
    <div className="app-layout">
        ...
        <div className="main-content" style={{ zIndex: 10 }}>
            ...
            <div className="scroll-area" ref={containerRef}>
                ...
                {tab === 'search' && (<div className="section">{resSongs.length > 0 && (<><div className="section-header">Songs from {APIs[source].name}</div><div className="grid">{resSongs.map((s, i) => (<div key={s.id} className="card" onClick={()=>playSong(resSongs, i)}><img src={getImg(s.image)} alt=""/><h3>{getName(s)}</h3><p>{getDesc(s)}</p><div className="card-actions"><button className={`btn-card-action ${isLiked(s.id)?'liked':''}`} onClick={(e)=>{e.stopPropagation(); toggleLike(s)}}><Icons.Heart/></button><button className="btn-card-action" onClick={(e)=>{e.stopPropagation(); setSongToAdd(s); setShowAddToPlaylistModal(true);}}><Icons.Plus/></button></div></div>))}</div></>)}</div>)}
                {tab === 'home' && (<><div className="hero"><h1>Welcome Back</h1><p>Discover.</p></div>{history.length > 0 && (<div className="section"><div className="section-header">Recently Played</div><div className="horizontal-scroll">{history.map((s, i) => (<div key={s.id} className="card" onClick={()=>playSong(history, i)}><img src={getImg(s.image)} alt=""/><h3>{getName(s)}</h3></div>))}</div></div>)}<div className="section"><div className="section-header">Moods</div><div className="horizontal-scroll">{MOODS.map(m => (<div key={m.id} className="card" style={{minWidth:'160px', background:m.color, display:'flex', alignItems:'center', justifyContent:'center'}} onClick={()=>handleCardClick(m, 'mood')}><h3 style={{fontSize:'1.2rem', color:'white', textAlign:'center'}}>{m.name}</h3></div>))}</div></div>{homeData.trending.length > 0 && (<div className="section"><div className="section-header">Trending Now</div><div className="horizontal-scroll">{homeData.trending.map((s, i) => (<div key={s.id} className="card" onClick={()=>handleCardClick(s, 'song')}><img src={getImg(s.image)} alt=""/><h3>{getName(s)}</h3><p>{getDesc(s)}</p></div>))}</div></div>)}</>)}
                {tab === 'library' && (<div className="section"><div className="section-header">Liked Songs</div><div className="grid">{likedSongs.map((s, i) => (<div key={s.id} className="card" onClick={()=>playSong(likedSongs, i)}><img src={getImg(s.image)} alt=""/><h3>{getName(s)}</h3><p>{getDesc(s)}</p><div className="card-actions" style={{opacity:1}}><button className="btn-card-action liked" onClick={(e)=>{e.stopPropagation(); toggleLike(s)}}><Icons.Heart/></button></div></div>))}</div></div>)}
            </div>
        </div>
